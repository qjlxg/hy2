    ...
    try:
        ...
        if scheme in ['vmess']:
            try:
                ...
                vmess_parsed_again = urlparse(scheme_specific_part)
                # Attempt to parse the payload assuming it's in the netloc or path
                payload_str = vmess_parsed_again.netloc if '@' not in vmess_parsed_again.netloc else vmess_parsed_again.netloc.split('@', 1)[1]
                if vmess_parsed_again.path and vmess_parsed_again.path.strip('/') != '':
                     payload_str = vmess_parsed_again.path.strip('/') # Sometimes payload is just the path after scheme://
                # If still looks like netloc, try splitting userinfo@host:port
                if ':' in payload_str and '@' in payload_str:
                     payload_str = payload_str.split('@', 1)[1] # Try getting host:port part
                     if ':' in payload_str:
                         payload_str = payload_str.split(':', 1)[0] # Try getting just host
                # If it's just the ID, it might not be valid JSON, need to be careful
                # Let's assume the decoded_payload from base64 step is the source of truth for JSON
                payload = json.loads(decoded_payload) # Use the payload decoded earlier
