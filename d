...
# --- 配置文件 ---
CONFIG_TG_TXT_FILE = 'configtg.txt'  # 纯文本输出
CONFIG_TG_YAML_FILE = 'configtg.yaml'  # YAML 输出
...
def generate_node_name(canonical_id):
    """基于 canonical_id 生成标准化的节点名称"""
    return hashlib.md5(canonical_id.encode('utf-8')).hexdigest()[:8]
...
def parse_and_canonicalize(link_string):
    ...
    try:
        ...
        if scheme == 'ssr' and ssr_decoded_parts:
            try:
                ...
                if '/?' in decoded_payload_str:
                    ssr_custom_query = str = decoded_payload_str.split('/?', 1)[1]
        ...
        if canonical_id:
            node_name = generate_node_name(canonical_id)
            return {
...
if __name__ == "__main__":
    ...
    for config in unique_configs.values():
        proxy = {
            ...
            'query': {k: v[0] if len(v) == 1 else v for k, v in config['query'].items()} or None,
